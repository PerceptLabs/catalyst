...
import * as sveltekitBundle from '../../catalyst-workers/test/fixtures/sveltekit-basic/.output/server/index.mjs';
...
describe('SvelteKit Adapter — Bindings', () => {
  ...
  it('both frameworks coexist in separate CatalystWorkers instances', async () => {
    const astroRuntime = await CatalystWorkers.create({
      workers: {
        astro: {
          module: (await import('../../catalyst-workers/test/fixtures/astro-basic/.output/server/index.mjs')) as unknown as WorkerModule,
          routes: ['/**'],
        },
      },
    });
